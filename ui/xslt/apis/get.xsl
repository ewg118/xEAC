...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:skos="http://www.w3.org/2004/02/skos/core#" xmlns:nm="http://nomisma.org/id/"
	...
	<xsl:template match="kml:Placemark" mode="kml-to-json">
		<xsl:variable name="coordinates" select="kml:Point/kml:coordinates"/>
		<xsl:variable name="start" select="if (kml:TimeStamp/kml:when) then kml:TimeStamp/kml:when else kml:TimeSpan/kml:begin"/>
		<xsl:variable name="end" select="kml:TimeSpan/kml:end"/>
		<xsl:variable name="theme">red</xsl:variable>
		
		{ <xsl:if test="string($coordinates)">"point": {"lon": <xsl:value-of
			select="normalize-space(tokenize($coordinates, ',')[1])"/>, "lat": <xsl:value-of select="normalize-space(tokenize($coordinates, ',')[2])"/>},</xsl:if> "title": "<xsl:value-of
				select="normalize-space(replace(kml:name, '&#x022;', '\\&#x022;'))"/>", <xsl:if test="string()">"start": "<xsl:value-of select="$start"
				/>",</xsl:if>
		<xsl:if test="string($end)">"end": "<xsl:value-of select="$end"/>",</xsl:if> "options": { "theme": "<xsl:value-of select="$theme"/>"<xsl:if
			test="string(kml:description)">, "description": "<xsl:value-of select="normalize-space(replace(kml:description, '&#x022;', '\\&#x022;'))"
			/>"</xsl:if> } } 
		<xsl:if test="not(position()=last())">
			<xsl:text>,</xsl:text>
		</xsl:if>
